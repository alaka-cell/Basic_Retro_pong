...
def render_menu(win, width, height, title_font, subtitle_font, score_font):
    ...
    # --- UI Element Heights for layout calculation ---
    spacing = 30
    ...
    # === Difficulty Buttons (Centered Group) ===
    ...
    # Pre-render all difficulty buttons and calculate total width
    ...
    for diff, text_surface, text_rect, box in button_boxes:
        ...
        if selected == diff:
            pygame.draw.rect(win, (255, 255, 255), box)  # filled box for selected
            text_surface = subtitle_font.render(diff, True, (0, 0, 0))  # black text
        else:
